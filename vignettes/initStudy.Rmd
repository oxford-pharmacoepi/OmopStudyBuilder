...
## Generating study code from a statistical analysis plan (SAP)

By default the analysis files in `studyCode/` are created empty, for you to
fill in. If the study already has a **statistical analysis plan** written with
the shinySAP application, you can pass its exported file to `initStudy()` with
the `sap` argument (a `.json` or `.jsonl` file, or the SAP already parsed as a
list):

* The study title, leads and description in the README default to the SAP's
  study information (explicit arguments still take precedence)
* `codelist/codelistCreation.R`, `cohorts/instantiateCohorts.R` and
  `analyses/incidencePrevalence.R` are generated from the SAP's codelists,
  cohorts and analyses
* `codeToRun.R` and `runStudy.R` are left untouched: they hold site-specific
  connection details
...
sap_root <- file.path(tempdir(), "SapStudy")

initStudy(sap_root, diagnostics = FALSE, sap = sap)
```

The cohort file is now filled in from the plan:

```{r sap-cohorts}
cat(readLines(file.path(sap_root, "studyCode", "cohorts", "instantiateCohorts.R")),
    sep = "\n")
```

The SAP names its codelists but does not carry their codes, so
`codelistCreation.R` reads each one from a CSV (`concept_id`, `concept_name`)
that you place under `codelist/<Category>/<name>.csv`; the folders are created
and each expected path is stated in the script.

If the SAP changes after the study has been created, regenerate the analysis
files in place with `generateStudyCode()`:

```{r sap-regenerate, message=FALSE}
generateStudyCode(sap, file.path(sap_root, "studyCode"))
```
