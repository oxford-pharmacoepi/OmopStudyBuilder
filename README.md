...
Initialize package version control for your study. OmopStudyBuilder
expects a `renv.lock` in your study folder (typically
`SampleStudy/study_code`).
...
Edit `code_to_run.R` in RStudio to set your database credentials (or use
a `.env` file and pass it with `env_file = ".env"`), then run the
...
`createStudy()` (including the `study_code/` project and its
`renv.lock`). Data partners can then build and run the study using
OmopStudyBuilder functions without needing to run Docker commands
manually.
