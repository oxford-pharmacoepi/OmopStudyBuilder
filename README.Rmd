...
To illustrate how OmopStudyBuilder works, we begin by loading the package:

```{r, eval = TRUE}
library(OmopStudyBuilder)
```

Next, we create a new study project. You can provide any study name; the example below creates `"SampleStudy"` in the current working directory:

```{r, eval = TRUE}
OmopStudyBuilder::createStudy("./SampleStudy")
```

After creating the project, open the study directory and use the [`renv`](https://rstudio.github.io/renv/) package to initialise a reproducible environment:

```{r, eval = FALSE}
renv::init()
```

This will generate an `renv.lock` file, which OmopStudyBuilder uses to analyse the study dependencies:
